...
namespace godot {
	...
	void FmodAudioStreamPlayer::set_stream(Ref<FmodAudioStream> new_stream) {
		...
		if (stream.is_valid()) {
			...
			internal_channel = system->play_sound(stream->sound, system->get_master_channel_group());
			if (!internal_channel) {
	...
	void FmodAudioStreamPlayer::play(double from_position) {
		if (!internal_channel) {
			return;
		}
	...
	void FmodAudioStreamPlayer::set_playing(bool play) {
		if (!internal_channel) {
			return;
		}
		playing = play;
